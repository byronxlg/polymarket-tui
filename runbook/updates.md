...
| Regenerate the demo's derived assets (`demo-poster.png`, `og.png`, `demo.gif`) | with every re-record - they are all frames of the cast | `uv run --with playwright python scripts/make_site_images.py`, then the `agg` line in `site/README.md` | the hero GIF and poster show the current UI |
