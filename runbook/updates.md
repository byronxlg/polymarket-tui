...
| Re-record `site/assets/demo.cast` | after visible UI changes | `bash scripts/record_demo.sh` (authed DRY, redacted; the script refuses to write if identity survives) | the cast plays locally with `asciinema play`; it is no longer on the page, it is the footage for the GIF and the launch video |
| Regenerate the demo's derived assets (`demo.gif`, `og.png`) | with every re-record - the GIF is frames of the cast, the og card is a screenshot of the hero | the `agg` line in `site/README.md`, then `uv run --with playwright python scripts/make_site_images.py` | the README hero GIF and the social card show the current UI |
| Regenerate the launch video (`site/assets/brag.mp4`, `brag.jpg`) | after re-recording the demo | `uv run --with playwright python scripts/make_brag_stills.py`, then re-render `brag/composition` (`npx hyperframes check && npx hyperframes render`), re-encode and copy to `site/assets/` (`site/README.md`); `/brag` only for a new story | the video plays on the landing page, README poster links to it |
